...
public interface RecommendationRepository extends Neo4jRepository<Recommendation, Long> {
    ...
    @Query("""
        ...
        WHERE u.id = $userId AND rating.rating >= $minRating
        ...
        AND COALESCE(m2.averageRating, 0.0) >= $minMovieRating
        RETURN DISTINCT
               m2 AS movie,
    ...
    @Query("MATCH (u1:User), (r1:Rating), (m1:Movie), (r2:Rating), (u2:User) " +
           ...
           "ORDER BY score DESC " +
           "LIMIT $limit")
    List<MovieRecommendationDTO> getCollaborativeRecommendations(@Param("userId") String userId,
                                                   @Param("minCommonRatings") Integer minCommonRatings,
                                                   @Param("minSimilarity") Double minSimilarity,
                                                   @Param("minRating") Double minRating,
                                                   @Param("limit") Integer limit);
    ...
    /**
     * Get movies rated highly by similar users with genre filter (collaborative filtering)
     * Filters recommended movies to only include specified genres
     */
    @Query("MATCH (u1:User), (r1:Rating), (m1:Movie), (r2:Rating), (u2:User) " +
            "WHERE u1.id = $userId " +
            "  AND r1.userId = u1.id AND r1.movieId = m1.id " +
            ...
            "ORDER BY score DESC " +
            "LIMIT $limit")
    List<MovieRecommendationDTO> getCollaborativeRecommendationsWithGenreFilter(
            @Param("userId") String userId,
            @Param("minCommonRatings") Integer minCommonRatings,
            @Param("minSimilarity") Double minSimilarity,
            @Param("minRating") Double minRating,
    ...
    @Query("MATCH (m:Movie) " +
           ...
           "ORDER BY score DESC " +
           "LIMIT $limit " +
           "RETURN movie, movieRating, score")
    List<MovieRecommendationDTO> getPopularRecommendations(@Param("userId") String userId,
                                            @Param("minRating") Double minRating,
                                            @Param("minRatingCount") Double minRatingCount,
                                            @Param("limit") Integer limit);

    @Query("""
        MATCH (u:User {id: $userId})-[r:RATED]->(m:Movie)
        WITH u, collect(m.id) AS watchedMovies
        MATCH (movie:Movie)-[:IN_GENRE]->(g:Genre)
        WHERE g.name IN $genres
            AND NOT movie.id IN watchedMovies
        OPTIONAL MATCH (movie)<-[rating:RATED]-(otherUser:User)
        WITH movie,
             AVG(rating.rating) AS avgRating,
             COUNT(rating) AS ratingCount
        WHERE avgRating >= $minRating
            AND ratingCount >= $minRatingCount
        WITH movie, avgRating, ratingCount,
             ((toFloat(ratingCount) / (ratingCount + $minRatingCount)) * avgRating +
              (toFloat($minRatingCount) / (ratingCount + $minRatingCount)) * 3.5) +
             log(ratingCount + 1) / log(1000) AS popularityScore
        RETURN movie, avgRating AS movieRating, popularityScore AS score
        ORDER BY popularityScore DESC
        LIMIT $limit
    """)
    List<MovieRecommendationDTO> getPopularRecommendationsByGenre(
            String userId,
            double minRating,
            double minRatingCount,
            List<String> genres,
            int limit
    );
    ...
    @Query("MATCH (u:User), (rating:Rating), (m1:Movie), (d:Director) " +
           ...
           "ORDER BY score DESC " +
           "LIMIT $limit")
    List<MovieRecommendationDTO> getDirectorBasedRecommendations(@Param("userId") String userId,
                                                   @Param("minRating") Double minRating,
                                                   @Param("minMovieRating") Double minMovieRating,
                                                   @Param("limit") Integer limit);

    // Genre-based recommendations WITH genre filter
    @Query("""
        MATCH (u:User)-[:RATED]->(rating:Rating)-[:RATED_MOVIE]->(m1:Movie)-[:IN_GENRE]->(g:Genre)
        WHERE u.id = $userId
          AND rating.rating >= $minRating
        ...
        AND COALESCE(m2.averageRating, 0.0) >= $minMovieRating
        AND EXISTS {
          MATCH (m2)-[:IN_GENRE]->(gFilter:Genre)
          WHERE gFilter.name IN $genres
        }
        RETURN DISTINCT
               m2 AS movie,
               COALESCE(m2.averageRating, 0.0) AS movieRating,
        ...
        LIMIT $limit
    """)
    List<MovieRecommendationDTO> getContentBasedRecommendationsWithGenreFilter(
            @Param("userId") String userId,
            @Param("minRating") double minRating,
            @Param("minMovieRating") double minMovieRating,
            @Param("genres") List<String> genres,
    ...
    // Director-based recommendations WITH genre filter
    @Query("""
        MATCH (u:User), (rating:Rating), (m1:Movie), (d:Director)
        WHERE u.id = $userId
          AND rating.userId = u.id AND rating.movieId = m1.id
          AND rating.rating >= $minRating
          AND (d)-[:DIRECTED]->(m1)
        ...
        LIMIT $limit
    """)
    List<MovieRecommendationDTO> getDirectorBasedRecommendationsWithGenreFilter(
            @Param("userId") String userId,
            @Param("minRating") Double minRating,
            @Param("minMovieRating") Double minMovieRating,
            @Param("genres") List<String> genres,
